...
class ResultsView(tk.Frame):
    ...
    def __init__(self, parent):
        ...
        self.yscrollbar.grid(row=1, column=2, rowspan=9, sticky="nse")
        self.xscrollbar = ttk.Scrollbar(self.base, orient=tk.HORIZONTAL)
        self.xscrollbar.grid(row=10, column=1, columnspan=2, sticky="swe")
        ...
        self.reportTxt.grid(row=1, column=1, rowspan=9, sticky="snew")
        ...
        ttk.Button(
            ...
            width=12,
            cursor="hand2",
            command=lambda: self.save("ready"),
        ).grid(row=1, column=3, sticky="snew", padx=30, pady=20)
        ttk.Button(
            self.base,
            text="save full",
            width=12,
            cursor="hand2",
            command=lambda: self.save("full"),
        ).grid(row=2, column=3, sticky="snew", padx=30, pady=10)
        ttk.Button(
            self.base,
            text="save mixed",
            width=12,
            cursor="hand2",
            command=lambda: self.save("mixed"),
        ).grid(row=3, column=3, sticky="snew", padx=30, pady=10)
        ttk.Button(
            self.base,
            text="save dups",
            width=12,
            cursor="hand2",
            command=lambda: self.save("dups"),
        ).grid(row=4, column=3, sticky="snew", padx=30, pady=10)
        ttk.Button(
            self.base,
            text="save missing",
            width=12,
            cursor="hand2",
            command=lambda: self.save("missing"),
        ).grid(row=5, column=3, sticky="snew", padx=30, pady=10)
