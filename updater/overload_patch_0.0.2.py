    ...
    try:
        ...
        for k, v in user_data['PlatformAPIs'].iteritems():
            ...
            if vault_secret == client_secret:
                patch_logger.info(
                    'Credentials for {} successfully stored in vault.'.format(
                        k))
            else:
                patch_logger.error(
                    'Credentials for {}={}:{} corrupted in the vault.'.format(
        ...
        patch_logger.warning(
            'Platform credentials not present or corrupted in this client.')

    except Exception as e:
        patch_logger.error('Encountered error: {}.'.format(sys.exc_info()))
